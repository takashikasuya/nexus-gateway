...
gateway ↔ Building OS の gRPC は Building OS の Envoy edge で mTLS 終端し、`gateway_id` を
...
| **高** | mTLS edge topology の実装確認 | [ADR-0007](adr/0007-transport-security-mtls-at-edge.md) で Building OS 側 Envoy / cert-manager / CN-SAN binding が外部依存として明記(Building OS #161) |
