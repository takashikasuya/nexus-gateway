...
- (resolved) The gateway→Building OS gRPC links are machine-authenticated by **mTLS terminated at the Building OS Envoy edge** (`gateway_id`↔client-cert CN/SAN), h2c in-cluster; no OIDC token on the link (ADR-0007). Keycloak/OIDC authenticates human operators at the Admin API only. Telemetry ingress is hosted by Building OS **ConnectorWorker** (`GatewayIngressService`), control egress by **GatewayBridge** (`GatewayEgressService`).
